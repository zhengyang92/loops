    ...
    struct Decorr *dpp)
{
  for (i = 0; i < nb_samples; i++)
	...
	((sam_B) !=
	 (short) (sam_B)
	 ? ((((((sam_B) & 0xffff) * (dpp->weightB)) >> 9) +
	     ((((sam_B) & ~0xffff) >> 9) * (dpp->weightB)) +
	     1) >> 1) : (((dpp->weightB) * (sam_B) + 512) >> 10));
      if ((sam_B) && (tmp))
	{
	  if (((sam_B) ^ (tmp)) < 0)
	    {
	...
	((sam_A) !=
	 (short) (sam_A)
	 ? ((((((sam_A) & 0xffff) * (dpp->weightA)) >> 9) +
	     ((((sam_A) & ~0xffff) >> 9) * (dpp->weightA)) +
	     1) >> 1) : (((dpp->weightA) * (sam_A) + 512) >> 10));
      if ((sam_A) && (tmp))
	{
	  if (((sam_A) ^ (tmp)) < 0)
	    {
