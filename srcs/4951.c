    ...
    int16_t * tmp)
{
  ...
  for (i = 0; i < w; i++)
	...
	(((dst[3 * dstStride]) +
      ...
      dst[4 * dstStride] =
	(((dst[4 * dstStride]) +
	  av_clip_uint8_c ((((tmp4 + tmp5) * 20 - (tmp3 + tmp6) * 5 +
			     (tmp2 + tmp7)) + 512) >> 10) + 1) >> 1);
      dst[5 * dstStride] =
	(((dst[5 * dstStride]) +
	  av_clip_uint8_c ((((tmp5 + tmp6) * 20 - (tmp4 + tmp7) * 5 +
			     (tmp3 + tmp8)) + 512) >> 10) + 1) >> 1);
      dst[6 * dstStride] =
	(((dst[6 * dstStride]) +
	  av_clip_uint8_c ((((tmp6 + tmp7) * 20 - (tmp5 + tmp8) * 5 +
			     (tmp4 + tmp9)) + 512) >> 10) + 1) >> 1);
      dst[7 * dstStride] =
	(((dst[7 * dstStride]) +
	  av_clip_uint8_c ((((tmp7 + tmp8) * 20 - (tmp6 + tmp9) * 5 +
			     (tmp5 + tmp10)) + 512) >> 10) + 1) >> 1);
      dst++;
      tmp++;
}}
