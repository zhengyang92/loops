...
int
fn (float t3, float *buf, const float icos36[9], float *out, float s0,
    float *win, int i, float s2, float s3, float tmp[18],
    const float icos36h[9], float t2, float t0, int j, float s1, float t1)
{
  for (j = 0; j < 4; j++)
    {
      t0 = tmp[i];
