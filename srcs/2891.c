...
int
fn (float t3, int i, float *buf, float *out, float s0, const float icos36[9],
    float s2, float s3, float *win, float tmp[18], float t1,
    const float icos36h[9], float t2, int j, float s1, float t0)
{
  for (j = 0; j < 4; j++)
    {
      t0 = tmp[i];
