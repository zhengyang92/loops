...
int
fn (const float srintensity, const float gintensity, const uint8_t roffset,
    const float scale, const float rintensity, const float rc, const float gc,
    const int depth, const float bc, const uint8_t goffset, const int width,
    const float max, const float bintensity, const float sbintensity,
    const float sgintensity, const uint8_t boffset, uint16_t * ptr,
    const int step)
{
