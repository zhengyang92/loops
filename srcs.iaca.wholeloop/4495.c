...
fn (uint16_t * const d1, const uint16_t * c0_data, const uint16_t * c2_data,
    const int d0_signed_linesize, uint16_t * const d2, const int c0_shift_w,
    const int slicew_end, int x, const int d1_signed_linesize,
    const int slicew_start, const int limit, const int c2_shift_w,
    const int d2_signed_linesize, uint16_t * const d0, const int c1_shift_w,
    const uint16_t * c1_data)
{
  IACA_START for (x = slicew_start; x < slicew_end; x++)
    {
      const int c0 =
