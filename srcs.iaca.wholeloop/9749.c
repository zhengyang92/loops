...
int
fn (const char *preset_name, AVIOContext ** s, int i, char filename[1000],
    int ret, const char *codec_name, const AVIOInterruptCB int_cb,
    const char *base[3])
{
  IACA_START for (i = 0; i < (sizeof (base) / sizeof ((base)[0])) && ret < 0; i++)
    {
      if (!base[i])
	continue;
      if (codec_name)
