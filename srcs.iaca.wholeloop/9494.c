...
fn (const uint8_t * scan, int max_coeffs, int bits_left,
    ...
    int blocks_per_slice, int run, int level, int16_t * out, int block_mask,
    int pos, GetBitContext * gb, int plane_size_factor,
    const uint8_t ff_prores_ac_codebook[7],
    const uint8_t ff_prores_lev_to_cb_index[10], int lev_cb_index,
    int run_cb_index)
{
  IACA_START for (pos = blocks_per_slice - 1; pos < max_coeffs;)
    {
