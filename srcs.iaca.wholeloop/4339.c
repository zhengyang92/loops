...
int
fn (uint8_t * ptr, const uint8_t roffset, const int step, const float scale,
    const float preserve, const float rc, const uint8_t boffset,
    const int width, const uint8_t goffset, const float gmw, const float rcw,
    const float by, const float byw, const float gm)
{
