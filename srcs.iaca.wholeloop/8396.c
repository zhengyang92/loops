...
int
fn (unsigned int rdiff, int thrb, unsigned int ldiff, unsigned int sum,
    int mid, unsigned int rsumdiff, int srcix, int thra,
    const uint16_t ** srcf, const int srcx, int size, int l, int srcjx,
    unsigned int lsumdiff)
{
  int r = 0;
  int x = 0;
  IACA_START for (int j = mid - 1, i = mid + 1; j >= 0 && i < size; j--, i++)
    {
      srcjx = srcf[j][x];
