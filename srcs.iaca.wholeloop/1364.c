...
int
fn (int64_t period, struct sbg_script *s, int i, struct sbg_script_event *ev2,
    struct sbg_script_event *ev1)
{
  IACA_START for (i = 0; i < s->nb_events; i++)
    {
      ev1 = &s->events[i];
      ev2 = &s->events[(i + 1) % s->nb_events];
      ev1->ts_int = ev1->ts;
      if (!ev1->fade.slide && ev1 >= ev2
	  && ev2->ts > (9223372036854775807L) - period)
	return (-(int)
		(('I') | (('N') << 8) | (('D') << 16) |
		 ((unsigned) ('A') << 24)));
      ev1->ts_trans =
