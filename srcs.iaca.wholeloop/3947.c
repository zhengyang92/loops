...
int
fn (double *param, int i, int64_t * filter, int xInc, int filterSize, int xx,
    int srcW, const int64_t fone, int64_t xDstInSrc, int j, int dstW,
    int flags)
{
  IACA_START for (j = 0; j < filterSize; j++)
    {
      int64_t d =
	(((((int64_t) xx * (1 << 17)) - xDstInSrc) >=
	  0 ? (((int64_t) xx * (1 << 17)) -
	       xDstInSrc) : (-(((int64_t) xx * (1 << 17)) -
			       xDstInSrc)))) << 13;
      double floatd;
      int64_t coeff;
      if (xInc > 1 << 16)
