...
int
fn (uint16_t * const pixels, int LUV, int UCS, int y,
    const struct ColorSystem *const cs, const uint16_t maxval, int leftEdge,
    const int linesize, const int cie, int x, const int w,
    const int correct_gamma, const float contrast, const float (*m)[3],
    int rightEdge, const int h, int XYY)
{
