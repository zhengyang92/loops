...
int
fn (uint16_t * const pixels, int UCS, int y, const float (*m)[3],
    const int correct_gamma, const int linesize, int leftEdge,
    const struct ColorSystem *const cs, const int cie, int x, const int w,
    const uint16_t maxval, int XYY, const float contrast, int rightEdge,
    const int h, int LUV)
{
