...
fn (x86_reg imm8OfPShufW2A, int numSplits, int16_t * filter, int i,
    ...
    x86_reg fragmentLengthB, int32_t * filterPos, int xpos,
  ...
  IACA_START for (i = 0; i < dstW / numSplits; i++)
      ...
      if ((i & 3) == 0)
	{
	  ...
	  x86_reg imm8OfPShufW1 =
	    (d + 1 < 4) ? imm8OfPShufW1B : imm8OfPShufW1A;
	  x86_reg imm8OfPShufW2 =
	    (d + 1 < 4) ? imm8OfPShufW2B : imm8OfPShufW2A;
	  x86_reg fragmentLength =
	    (d + 1 < 4) ? fragmentLengthB : fragmentLengthA;
