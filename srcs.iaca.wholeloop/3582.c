    ...
    uint16_t * dst, ptrdiff_t stridea)
{
  IACA_START for (i = 0; i < 8; i++, dst += stridea)
      ...
      else
	{
	  ...
	  if (hev)
	    {
	      int f = av_clip_intp2_c (p1 - q1, 12 - 1), f1, f2;
	      f = av_clip_intp2_c (3 * (q0 - p0) + f, 12 - 1);
	      f1 =
		((f + 4) >
		 ((1 << (12 - 1)) - 1) ? ((1 << (12 - 1)) - 1) : (f +
								  4)) >> 3;
		...
		((f + 3) >
		 ((1 << (12 - 1)) - 1) ? ((1 << (12 - 1)) - 1) : (f +
								  3)) >> 3;
	      dst[strideb * -1] = av_clip_uintp2_c (p0 + f2, 12);
	      dst[strideb * +0] = av_clip_uintp2_c (q0 - f1, 12);
	    }
	  else
	    {
	      int f = av_clip_intp2_c (3 * (q0 - p0), 12 - 1), f1, f2;
	      f1 =
		((f + 4) >
		 ((1 << (12 - 1)) - 1) ? ((1 << (12 - 1)) - 1) : (f +
								  4)) >> 3;
		...
		((f + 3) >
		 ((1 << (12 - 1)) - 1) ? ((1 << (12 - 1)) - 1) : (f +
	      ...
	      dst[strideb * +0] = av_clip_uintp2_c (q0 - f1, 12);
	      f = (f1 + 1) >> 1;
	      dst[strideb * -2] = av_clip_uintp2_c (p1 + f, 12);
	      dst[strideb * +1] = av_clip_uintp2_c (q1 - f, 12);
}IACA_END}}}
