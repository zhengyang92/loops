...
int
fn (OpusRangeCoder * rc, int i,
    const uint16_t silk_model_excitation_sign[3][2][7][3], int active,
    int voiced, int32_t excitation[320], int shellblocks, int qoffset_high,
    uint8_t pulsecount[20])
{
  IACA_START for (i = 0; i < shellblocks << 4; i++)
    {
      if (excitation[i] != 0)
	{
	  int sign =
