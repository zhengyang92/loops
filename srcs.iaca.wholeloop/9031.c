...
int
fn (int mrefs, int w, int x, uint16_t * prev, int prefs, const int edge,
    uint16_t * next2, int mode, uint16_t * next, uint16_t * cur,
    uint16_t * dst, uint16_t * prev2)
{
  IACA_START for (x = w - edge; x < w - 3; x++)
    {
      int c = cur[mrefs];
