...
int
fn (int msk_val[50], const uint16_t * fast_decay, int i, int slow_decay,
    int fast_gain, const uint16_t * slow_gain, int psd_val[50],
    const uint16_t * fast_gain_adj, int nb_exponent)
{
  int fast_leak = 0;
  int slow_leak = 0;
  IACA_START for (i = 0; i < nb_exponent; i++)
    {
      fast_leak =
