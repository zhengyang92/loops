...
int
fn (int mrefs, int w, uint8_t * cur, uint8_t * next2, int x, int prefs,
    uint8_t * next, uint8_t * prev2, uint8_t * prev, int mode, uint8_t * dst)
{
  IACA_START for (x = 0; x < ((3) > (w) ? (w) : (3)); x++)
    {
      int c = cur[mrefs];
