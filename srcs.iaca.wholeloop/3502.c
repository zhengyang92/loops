...
int
fn (float codebook_delta_value, float codebook_minimum_value,
    uint16_t * codebook_multiplicands, unsigned int codebook_value_bits,
    unsigned int i, unsigned int codebook_lookup_values, GetBitContext * gb)
{
  IACA_START for (i = 0; i < codebook_lookup_values; ++i)
    {
