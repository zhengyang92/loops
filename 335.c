...
fn (int w, const int rnd, ptrdiff_t s, int cgu, int cgv,
    ...
    const int sh, const int16_t * rgb1, unsigned int mask,
    const int16_t * rgb2, int cgy, int y, uint16_t * yuv2, int cru,
    uint16_t * yuv1, const int16_t * rgb0, int *(*rnd_scratch)[2], int cburv,
    const int uv_offset, ptrdiff_t s0, int cbv)
{
  for (x = 0; x < w; x++)
	...
	r00 * cry + g00 * cgy + b00 * cby + rnd_scratch[0][y & !1][x << 1];
      diff = (y00 & mask) - rnd;
      yuv0[x << 1] = av_clip_uintp2_c (yuv_offset[0] + (y00 >> sh), 10);
	...
	r01 * cry + g01 * cgy + b01 * cby + rnd_scratch[0][y & !1][x * 2 + 1];
      diff = (y01 & mask) - rnd;
      yuv0[x * 2 + 1] = av_clip_uintp2_c (yuv_offset[0] + (y01 >> sh), 10);
      ...
      diff = (y10 & mask) - rnd;
      yuv0[x * 2 + 0 + s0] =
	av_clip_uintp2_c (yuv_offset[0] + (y10 >> sh), 10);
      ...
      diff = (y11 & mask) - rnd;
      yuv0[x * 2 + 1 + s0] =
	av_clip_uintp2_c (yuv_offset[0] + (y11 >> sh), 10);
	...
	rnd_scratch[1][y & 1][x];
      diff = (u & mask) - rnd;
      yuv1[x] = av_clip_uintp2_c (uv_offset + (u >> sh), 10);
	...
	rnd_scratch[2][y & 1][x];
      diff = (v & mask) - rnd;
      yuv2[x] = av_clip_uintp2_c (uv_offset + (v >> sh), 10);
