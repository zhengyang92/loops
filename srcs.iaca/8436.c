...
fn (uint8_t * ptr, const int step, const uint8_t roffset, const float scale,
    const float preserve, const float rc, const uint8_t goffset,
    const int width, const float gmw, const uint8_t boffset, const float by,
    const float byw, const float rcw, const float gm)
{
