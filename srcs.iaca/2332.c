...
fn (int32_t * block, uint16_t * dst, int i, int stride)
{
  for (i = 0; i < 8; i++)
	...
	(block[1 + i * 8] >> 1);
      ...
      dst[i + 0 * stride] =
	av_clip_uintp2_c (dst[i + 0 * stride] + ((int) (b0 + b7) >> 6), 14);
      dst[i + 1 * stride] =
	av_clip_uintp2_c (dst[i + 1 * stride] + ((int) (b2 + b5) >> 6), 14);
      dst[i + 2 * stride] =
	av_clip_uintp2_c (dst[i + 2 * stride] + ((int) (b4 + b3) >> 6), 14);
      dst[i + 3 * stride] =
	av_clip_uintp2_c (dst[i + 3 * stride] + ((int) (b6 + b1) >> 6), 14);
      dst[i + 4 * stride] =
	av_clip_uintp2_c (dst[i + 4 * stride] + ((int) (b6 - b1) >> 6), 14);
      dst[i + 5 * stride] =
	av_clip_uintp2_c (dst[i + 5 * stride] + ((int) (b4 - b3) >> 6), 14);
      dst[i + 6 * stride] =
	av_clip_uintp2_c (dst[i + 6 * stride] + ((int) (b2 - b5) >> 6), 14);
      dst[i + 7 * stride] =
	av_clip_uintp2_c (dst[i + 7 * stride] + ((int) (b0 - b7) >> 6), 14);
}}
