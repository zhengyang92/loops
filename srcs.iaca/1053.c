...
int
fn (const float srintensity, uint8_t * ptr, const int step,
    const float rintensity, const float scale, const float rc, const float gc,
    const float bc, const int width, const uint8_t goffset,
    const float bintensity, const float sgintensity, const float sbintensity,
    const float gintensity, const uint8_t boffset, const uint8_t roffset)
{
