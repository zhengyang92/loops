...
int
fn (uint8_t * ptr, const float gm, const uint8_t roffset, const float scale,
    const float preserve, const float rc, const uint8_t boffset,
    const uint8_t goffset, const int width, const float gmw, const float rcw,
    const float by, const float byw, const int step)
{
