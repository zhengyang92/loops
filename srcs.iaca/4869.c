...
int
fn (int alpha, int d, uint16_t * pix, const int tc_orig, ptrdiff_t ystride,
    int inner_iters, int beta, ptrdiff_t xstride)
{
  for (d = 0; d < inner_iters; d++)
	  ...
	  && ((q1 - q0) >= 0 ? (q1 - q0) : (-(q1 - q0))) < beta)
	{
	  ...
	  pix[-xstride] = av_clip_uintp2_c (p0 + i_delta, 12);
	  pix[0] = av_clip_uintp2_c (q0 - i_delta, 12);
	}
      pix += ystride;
     } IACA_END
}
