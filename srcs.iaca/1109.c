...
int
fn (uint16_t * d1_data, const uint16_t * c0_data, const int intensity,
    const uint16_t * c2_data, const int mid, const int src_w,
    uint16_t * d2_data, int x, const int c0_shift_w, const int limit,
    const int c2_shift_w, uint16_t * d0_data, const int c1_shift_w,
    const int max, const uint16_t * c1_data)
{
  for (x = 0; x < src_w; x++)
    {
      const int c0 =
