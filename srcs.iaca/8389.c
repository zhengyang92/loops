...
int
fn (int cvv, int w, const int rnd, int cyv, int y_off_in, uint16_t * dst2,
    int cyu, int cuv, int y_off_out, int x, const uint16_t * src2,
    const int sh, uint16_t * dst0, const ptrdiff_t *src_stride, int cuu,
    const ptrdiff_t *dst_stride, int cvu, uint16_t * dst1,
    const int uv_off_in, const uint16_t * src1, const uint16_t * src0,
    const int uv_off_out, int cyy)
{
  for (x = 0; x < w; x++)
    {
	...
	av_clip_uintp2_c ((cyy * y10 + uv_val) >> sh, 10);
      dst0[x * 2 + 1 + dst_stride[0] / sizeof (uint16_t)] =
	av_clip_uintp2_c ((cyy * y11 + uv_val) >> sh, 10);
