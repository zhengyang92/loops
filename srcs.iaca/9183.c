...
int
fn (float tmp10, const float postscale[64], float tmp4, float tmp13,
    float tmp5, int i, float tmp7, float tmp6, float tmp1, int16_t * data,
    float tmp12, float tmp0, float temp[64], float tmp2, float tmp3,
    float tmp11)
{
  for (i = 0; i < 8; i++)
    {
