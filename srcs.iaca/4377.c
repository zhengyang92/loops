...
int
fn (int i, const int fill_color, const uint8_t * indata, const uint64_t max,
    const int depth, const int w, const int off_y, const int inlinesize,
    const uint64_t add, const int xcenter, const int ycenter, const int h,
    uint8_t * out, const int32_t * correction)
{
  for (int j = 0; j < w; j++)
     { IACA_START
