...
int
fn (const float srintensity, const int step, const float rintensity,
    const float scale, const float rc, const float gc, const uint8_t boffset,
    const float bc, const int width, const uint8_t goffset, const int depth,
    const float max, const float sgintensity, const float sbintensity,
    const float bintensity, const float gintensity, uint16_t * ptr,
