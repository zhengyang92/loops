...
int
fn (const float srintensity, uint8_t * ptr, const uint8_t roffset,
    const float scale, const float rintensity, const float rc, const float gc,
    const float bc, const uint8_t goffset, const int width,
    const float bintensity, const float sbintensity, const float sgintensity,
    const float gintensity, const uint8_t boffset, const int step)
{
