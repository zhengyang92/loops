...
fn (int32_t * block, uint16_t * dst, int i, int stride)
{
  for (i = 0; i < 4; i++)
	...
	block[1 + 4 * i] + (unsigned) (block[3 + 4 * i] >> 1);
      dst[i + 0 * stride] =
	av_clip_uintp2_c (dst[i + 0 * stride] + ((int) (z0 + z3) >> 6), 12);
      dst[i + 1 * stride] =
	av_clip_uintp2_c (dst[i + 1 * stride] + ((int) (z1 + z2) >> 6), 12);
      dst[i + 2 * stride] =
	av_clip_uintp2_c (dst[i + 2 * stride] + ((int) (z1 - z2) >> 6), 12);
      dst[i + 3 * stride] =
	av_clip_uintp2_c (dst[i + 3 * stride] + ((int) (z0 - z3) >> 6), 12);
} IACA_END }
