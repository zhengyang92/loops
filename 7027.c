...
int
fn (int Fd, int Dd, ptrdiff_t stride, int Bdd, int16_t * ip, int E, int Cd,
    ...
    int type, int Add, int i, int H, int Ed, int F, uint8_t * dst)
{
  for (i = 0; i < 8; i++)
      ...
      if (ip[1] | ip[2] | ip[3] | ip[4] | ip[5] | ip[6] | ip[7])
	{
	  ...
	  B = (((12785) * (ip[1])) >> 16) - (((64277) * (ip[7])) >> 16);
	  C = (((54491) * (ip[3])) >> 16) + (((36410) * (ip[5])) >> 16);
	  D = (((54491) * (ip[5])) >> 16) - (((36410) * (ip[3])) >> 16);
	  Ad = (((46341) * ((A - C))) >> 16);
	  Bd = (((46341) * ((B - D))) >> 16);
	  Cd = A + C;
	  Dd = B + D;
	  E = (((46341) * ((ip[0] + ip[4]))) >> 16) + 8;
	  F = (((46341) * ((ip[0] - ip[4]))) >> 16) + 8;
	  if (type == 1)
	    {
	      E += 16 * 128;
	      F += 16 * 128;
	    }
	  G = (((60547) * (ip[2])) >> 16) + (((25080) * (ip[6])) >> 16);
	  H = (((25080) * (ip[2])) >> 16) - (((60547) * (ip[6])) >> 16);
	  Ed = E - G;
	  Gd = E + G;
	  Add = F + Ad;
	  Bdd = Bd - H;
