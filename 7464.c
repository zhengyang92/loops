...
int
fn (int mrefs, int w, int interpol, int spat, int x, uint16_t * prev,
    int prefs, int mrefs2, uint16_t * next2, uint16_t * cur, uint16_t * next,
    int clip_max, uint16_t * dst, uint16_t * prev2, int prefs2)
{
  for (x = 0; x < w; x++)
    {
      int c = cur[mrefs];
